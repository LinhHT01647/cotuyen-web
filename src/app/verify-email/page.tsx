...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        ...
        if (res.ok) {
          // Lưu tên nếu backend trả về username
          if (data.username) {
            setUsername(data.username);
            localStorage.setItem("username", data.username);
          }
          setStatus("success");
          // Tự chuyển về trang đăng nhập sau 5 giây
  ...
  return (
    <main
      ...
      style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(218,0,0,0.08) 0%, #080404 60%)" }}
    >
      {/* Logo */}
      <Link href="/" className="mb-12 flex items-center gap-2">
        <span className="heading-font text-3xl font-black tracking-widest" style={{ color: "#DA0000" }}>
          CỜ TUYẾN
        </span>
      </Link>
      ...
      >
        ...
        {status === "success" && (
          <>
            <div className="w-24 h-24 rounded-full bg-green-900/30 border border-green-500/30 flex items-center justify-center mb-8" style={{ boxShadow: "0 0 30px rgba(34,197,94,0.15)" }}>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-center gap-2 mb-3">
                ...
                <span className="text-xs text-[#DA0000] heading-font font-bold tracking-widest uppercase">EMAIL ĐÃ XÁC MINH</span>
              </div>
              ...
              <p className="text-gray-400 text-sm leading-relaxed mt-3">
                ...
                <br />Đang chuyển đến trang đăng nhập trong <strong className="text-white">5 giây</strong>...
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 w-full gap-3 mb-8">
              {[
                ...
                <div key={item.label} className="bg-black/30 rounded p-3 border border-white/5">
                  <p className="text-[9px] text-gray-500 heading-font tracking-widest mb-1">{item.label}</p>
